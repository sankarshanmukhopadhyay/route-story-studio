...
test('poster distinguishes estimated ascent from recorded elevation gain',()=>{const base={source:{name:'x'},segments:[{points:[{latitude:1,longitude:1,elevationMetres:1},{latitude:2,longitude:2,elevationMetres:2}]}]};const statistics={distanceMetres:1000,elevationGainMetres:10,durationSeconds:null};assert.match(createPosterSvg({route:{...base,sourceType:'planned-route'},statistics,title:'x'}),/ESTIMATED ASCENT/);assert.match(createPosterSvg({route:{...base,sourceType:'recorded-track'},statistics,title:'x'}),/ELEVATION GAIN/);});
